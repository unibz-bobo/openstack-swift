...
do
    echo "    server s$i $device:8080 maxconn 512" >> load_balancer/haproxy.cfg.tmp
    i=$((i+1))
done
...
ssh -i stack_rsa -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no root@$LOAD_BALANCER "killall haproxy ; haproxy -f /etc/haproxy/haproxy.cfg -D" >> logs/$LOAD_BALANCER 2>&1
